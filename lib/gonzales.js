...
/***/ (function(module, exports, __webpack_require__) {
	...
	function getDimension() {
	  ...
	  var unit = tokens[pos];
	  var ident = newNode(NodeType.IdentType, getNmName2(), unit.ln, unit.col);
	...
	*/
	...
	function getPseudoClass3() {
...

	  {
	    var _l = tokens[pos].ln;
	    var _c = tokens[pos].col;
	    var _content2 = tokens[pos].value;
	...
	function getStylesheet() {
	  ...
	  while (pos < tokensLength) {
	    if (wasDeclaration && checkDeclDelim(pos)) node = getDeclDelim();else if (checkSC(pos)) node = getSC();else if (checkRuleset(pos)) node = getRuleset();else if (checkInclude(pos)) node = getInclude();else if (checkExtend(pos)) node = getExtend();else if (checkMixin(pos)) node = getMixin();else if (checkLoop(pos)) node = getLoop();else if (checkConditionalStatement(pos)) node = getConditionalStatement();else if (checkAtrule(pos)) node = getAtrule();else if (checkDeclaration(pos)) node = getDeclaration();else throwError();
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function checkTset(i) {
	  ...
	  if (l = checkVhash(i)) tokens[i].tset_child = 1;else if (l = checkOperator(i)) tokens[i].tset_child = 2;else if (l = checkAny(i)) tokens[i].tset_child = 3;else if (l = checkSC(i)) tokens[i].tset_child = 4;else if (l = checkInterpolation(i)) tokens[i].tset_child = 5;
